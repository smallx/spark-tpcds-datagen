WITH year_total AS (
  SELECT
    ...
    's' sale_type
  FROM ${database}.customer, ${database}.store_sales, ${database}.date_dim
  ...
  SELECT
    ...
    'c' sale_type
  FROM ${database}.customer, ${database}.catalog_sales, ${database}.date_dim
  ...
  SELECT
    ...
    sum((((ws_ext_list_price - ws_ext_wholesale_cost - ws_ext_discount_amt) + ws_ext_sales_price) /
      2)) year_total,
    'w' sale_type
  FROM ${database}.customer, ${database}.web_sales, ${database}.date_dim
